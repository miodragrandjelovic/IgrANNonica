...
class Model():
    ...
    def trainModel(self,type,epochs, batch_size,path):
        # train our model
        self.pred,self.label,self.eveluate,self.history = fn.train_model(self.model,type, self.data.X_train, self.data.y_train, epochs, batch_size,self.data.X_val,self.data.y_val, self.data.X_test, self.data.y_test,path)
