...
class Statistics():
    ...
    def createModel(self,train, features, label, epochs, ratio, activation_function_list, hidden_layers_n, hidden_layer_neurons_list, columns,enc_types,num_cat_col,randomize, batch_size, learning_rate, regularization, regularization_rate):
        ...
        data=None
        data=pr.Data(train)
        data.Misa(columns,enc_types,num_cat_col,self.type,features,label,ratio,randomize)
